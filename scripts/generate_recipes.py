...
def generate_vendor_prices_lua(entries: list[dict], flavor: str) -> str:
    """Emit Data/<Flavor>/VendorPrices.lua text for the computed entries."""
    is_sod = (flavor == "SOD")
    folder = "SoD" if is_sod else "TBC"
    lines = [
    ...
    if is_sod:
        lines.append('    flavor = "SOD",  -- DEFAULT (TBC) bucket omits this; registers only on a SoD client')
    lines.append("    prices = {")
    for e in entries:
        lines.append(
...
def emit_vendor_prices(item_details: dict, flavor: str, exp_name: str, output_dir: Path,
    ...
    lua = generate_vendor_prices_lua(entries, flavor)
...
def main() -> int:
    ...
    if args.vendor_prices_only:
        flavor_token = "SOD" if is_sod else "DEFAULT"
        emit_vendor_prices(load_item_details(data_dir), flavor_token, exp_name, args.output_dir)
        return 0
